...
key = "ERB7wTkKTDx8mEvQgcAiJA3zjAjEpK7Q"
secret = "oUI3uI77OK0oaVmz5VO3zUWxC9LIsD75"

def jiance():   #摄像头实时分析
    http_url = "https://api-cn.faceplusplus.com/facepp/v3/detect"
    filepath1 = "zln.jpg"
    # 需要发送的数据
    data = {"api_key": key, "api_secret": secret, "return_attributes": "age,emotion"}
    files = {"filepath": open(filepath1,"rb")}
    cap = cv2.VideoCapture(0)
    while True:
        ret,frame = cap.read()
        cv2.imshow("capture",frame)
        cv2.imwrite("zln.jpg",frame)
        ...
        def Compare():  # 人脸对比
            ...
            # 需要发送的数据
            data = {"api_key": key, "api_secret": secret, "face_token1": "8b7fa9baf07b6bb1b4e7a22aec64e22d",
                    "face_token2": "1b14251fdc701551e025b152dc272f33"}  # 多个返回值用“,”隔开
            image_file2 = "zln.jpg.jpg"
        ...
        def FaceSet_AddFace():  # 添加人脸到库中
            ...
            data = {"api_key": key, "api_secret": secret, "faceset_token": "5309ce72084fe8430a6ca31b4a700bc4",
                    "face_tokens": "8b7fa9baf07b6bb1b4e7a22aec64e22d", }
            response = requests.post(url, data=data)
            test = response.json()
            print(test)
        ...
        def camera_compare():  # 摄像头实时比对
            ...
            # 需要发送的数据
            data = {"api_key": key, "api_secret": secret,
                    "face_token1": "8b7fa9baf07b6bb1b4e7a22aec64e22d"}  # 多个返回值用“,”隔开
            files = {"image_file": open(filepath1, "rb")}
            cap = cv2.VideoCapture(0)
            while True:
                ret, frame = cap.read()
